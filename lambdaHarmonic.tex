...
\begin{theorem}\label{almostHarmSGD}
  Let $\mathcal{M} = \R^{d}$ and $w_1,...,w_k$ be in $\R^d$ with $\|w_i\| \leq \log(d)$. Let $b_1,...,b_k$ be randomly chosen at uniform from $[-1,1]$. 
  ...
  $G(\boldsymbol{a,\theta})$ such that either 1) $G(\boldsymbol{a,\theta}) \leq \epsilon$ or 2) there exists an $i, j$ such that $\|\theta_i - w_j\| <  \epsilon$ in $poly(d,1/\epsilon)$ iteration complexity.
\end{theorem}
...
Otherwise, our subset $S$ does not exist. 
...
Because $\|w_j\| \leq \log(d)$, we see that $\Phi({\bf 0}, w_j) \geq 1/\poly(d)$. And since $b_j$ are uniformly chosen in $[-1,1]$, we conclude that we high probability over the choices of $b_j$, $-\frac{1}{2}\left(  \sum_{j=1}^k b_j\Phi(\theta_1,w_j)\right)^2 \geq 1/\poly(d)$.  

Therefore, we conclude that with high probability, $G(a_1, {\bf 0}) \leq G(\boldsymbol{0,0}) - 1/\poly(d)$. Since $G(\boldsymbol{0,0}) \geq \epsilon$, we derive our claim.
...
Lemma~\ref{strongConvergeTwo} (with more arguments on Lipschitz constants etc) ensures that we are in $\mathcal{M}_{G,\poly(\epsilon,1/d)}$ in $\poly(d,1/\epsilon)$ iterations.
If $G(\boldsymbol{0,0}) \leq \epsilon$, then by Lemma~\ref{strongConvergeTwo}, with high probability, $G(\boldsymbol{a,\theta}) \leq G(\boldsymbol{0,0}) \leq \epsilon$. Otherwise, by Lemma \ref{almostHarmInitialize},  we can initialize $\boldsymbol{a^{(0)},\theta^{(0)}}$ such that $G(\boldsymbol{a^{(0)},\theta^{(0)}}) \leq  G(0,0) - 2\delta \sqrt{G(0,0)}$ for $\delta = 1/\poly(d,1/\epsilon)$. Finally, we conclude by Theorem \ref{almostHarmRes}.
\end{proof}
