...
\begin{observation}
Notice that upon initialization, we have $L(\boldsymbol{a,\theta}) < L(0,\boldsymbol{\theta})$ almost surely since ${\bf a}$ can be optimized to make the loss strictly smaller (look at \ref{initialize}). Continuous gradient descent on $\lambda$-harmonic functions will never have $\hat{f} = 0$ since the loss function is non-increasing.
\end{observation}
...
\end{theorem}
\Snote{Does this theorem need to be here?}\Rnote{Right im not sure if we should include this theorem at all? or where to put}
...
& \geq \|f(x)\|_X\ - \sum_{j} \|\sum_{i\in S_j}a_i\sigma(\theta_i,x)
  \|_X \geq \sqrt{L(0,\boldsymbol{\theta})} - \epsilon
\end{align*}
