...
\end{align*}
Where $k_m = m^2 + (d-2)m + 1$. Notice that $f(x)$ has \Snote{This is not $f$ anymore} non-negative
Taylor coefficients and is a bounded polynomial, so $f$ is a
realizable potential function by Theorem \ref{thm:rotReal}, with
corresponding $\sigma(x) = \sum_{i} \sqrt{c_i} H_i(x)$, where $H_i(x)$
...
\label{eigConv}
Let $\mathcal{M} = S^{d-1}$ and let $b_1,...,b_k$ be reals bounded by $poly(d)$. Furthermore, let $L$ be as in \eqref{errLoss} with potential $\Phi_m$ and let $G = L + \|a\|^2$. 

For all $ 1 > \epsilon > 0$, there exists $m = O(poly(d)/\epsilon)$ such that if $(\boldsymbol{a,\theta}) \in \mathcal{M}_{poly(\epsilon,1/d)}$ and $G(\boldsymbol{a,\theta}) \leq G(0,\boldsymbol{\theta})$, then for all $i$, either there exists $j$ such that $|w_j^T\theta_i| > 1- \epsilon$ or $|a_i| < \epsilon/k$. 
\end{restatable}
...
weaker result: each $(a_i,\theta_i)$ must satisfy (i) there exists $j$
such that $|w_j^T\theta_i| > 1-\epsilon$, or (ii) $|a_i| < \epsilon$,
or (iii) there exists $j\neq i$ such that $|\theta_j^T\theta_i| >
1-\epsilon$. Assume that $(\boldsymbol{a,\theta}) \in
\mathcal{M}_{\epsilon^2/d}$ and we have a $(a_i,\theta_i)$ that does
not satisfy (i), (ii), (iii). Then,
%
...
%
\begin{align*}
\Delta_{\theta_i} G & =  2\sum_{j=1}^k a_i b_j  (-\Phi(\theta_i^T w_j)  
+ k_m(\theta_i^Tw_j)^m)  \\
& \qquad +  2\sum_{j: j\neq i} a_i
...
%
\begin{align*}
& |\Delta_{\theta_i}G + (2\Phi(1) + 2)a_i^2| \leq \epsilon |a_i|  \\
...
%
\begin{align*}
\Delta_{\theta_i} G \leq -(2\Phi(1) + 2)a_i^2 +\epsilon |a_i|+
...
Let $(\boldsymbol{a,\theta}) \in \mathcal{M}_{\epsilon^2/(4d)}$. Then,
the optimality conditions on ${\bf a}$ are unchanged, we must have
%
\begin{align*}
\Delta_{{\bf v}} H & =  2\sum_{i=1}^l\sum_{j=1}^k a_i b_j  (-\Phi(\theta_i^T w_j) + k_m(\theta_i^Tw_j)^m) \\
& \quad + 2\sum_{i=1}^l\sum_{j= l+1}^k a_i
a_j(-\Phi(\theta_i^T\theta_j) + k_m(\theta_i^T\theta_j)^m)
\end{align*}
...
%
\begin{align*}
&  \abs{\Delta_{{\bf v}}H + \sum_{i=1}^l (2\Phi(\theta_i^T\theta_i)+2)a_i^2
  + 2\sum_{i,j: i \neq j}^l a_ia_j\Phi(\theta_i^T\theta_j)}  \\ 
& \leq \epsilon |a_i| 
+  2 k_m |a_i| \sum_{i=1}^l \abs{\sum_{j=1}^k b_j (\theta_i^Tw_j)^m
...
%
\begin{align*}
\Delta_{\theta_i} G & \leq -2a_i^2 +\epsilon |a_i|+(\epsilon/2)|a_i| \\
...
Assume the conditions of \ref{eigConv}. If $\sqrt{G({\bf a, \boldsymbol{\theta}})} < \sqrt{G(0,\boldsymbol{\theta})} - \epsilon$ and $(\boldsymbol{a,\theta}) \in \mathcal{M}_{poly(\epsilon,1/d)}$, then there exists some $i, j$ such that $\theta_i$ is in an $\epsilon$-neighborhood of $w_j$.
\end{theorem}
...
triangle inequality
\begin{align*}
\sqrt{G(\boldsymbol{a,\theta})} & = \|\sum_{i=1}^k a_i \sigma(\theta_i,x) - f(x)\|_X \\
& \geq \|f(x)\|_X\ - \sum_{i=1}^k \|a_i\sigma(\theta_i,x) \|_X \geq
  \sqrt{G(0,\boldsymbol{\theta})} - \epsilon
\end{align*}
...
\[ \expt\left[\left(  \sum_{j=1}^k b_j \Phi(\theta,w_j)\right)^2\right] > 5\epsilon\sqrt{L(0,0)}\] 

Then, we can initialize $\boldsymbol{a,\theta}$ such that $\sqrt{G({\bf a, \boldsymbol{\theta}})} < \sqrt{G(0,\boldsymbol{\theta})} - \epsilon$ with high probability.
 \end{observation}
...
Let $h(\theta) = \sum_i b_i \Phi(\theta, w_j)$. Since $a_i,b_i, \Phi$ is $poly(d)$-bounded, our random variable $g(\theta)$ is $poly(d)$-bounded. From Hoeffding bounds, we see that after $poly(d,1/\epsilon)$ samples, we know that $\hat{\expt}h(\theta)^2 \geq 2\epsilon\sqrt{G(0,0)}$, where $\hat{\expt}$ denotes the empirical average over $\theta$. Therefore, we must have found some $\theta$ such that $h(\theta)^2 \geq 2\epsilon \sqrt{G(0,0)}$. Therefore, $G(\boldsymbol{a,\theta}) - G(0,0) \leq 2\epsilon \sqrt{G(0,0)}$ and so $\sqrt{G(\boldsymbol{a,\theta})} \leq \sqrt{G(\boldsymbol{0,\theta})} - \epsilon$.
 \end{proof}
