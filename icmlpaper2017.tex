...
\begin{theorem}
  ...
  such that for the $m$-almost $1$-harmonic potential $\Phi_m$ the
  following holds: we can chose an initial point
  $(\boldsymbol{a^{(0)}, \theta^{(0)}})$ so that if after running
  gradient descent on the regularized objective
  $G(\boldsymbol{a,\theta}),$ we have
  $(\boldsymbol{a,\theta}) \in \mathcal{M}_{\delta^2/(4d)},$ then
  there exists an $i, j$ such that $|w_j^T\theta_i| > 1- \epsilon,$
% with high
%   probability after running Gradient Descent (Algorithm 1) 
\end{theorem}
\begin{proof}
  Follows by combining Theorem~\ref{eigConv},
  Observation~\ref{initialize} and Lemma~\ref{variance}, and
  replacing $\epsilon$ by $\epsilon/k^2.$
\end{proof}
