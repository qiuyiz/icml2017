...
\begin{algorithmic}
  ...
  \FOR{$i=1$ {\bfseries to} $k$} \REPEAT \STATE Sample $\theta_i$
...
(notice $\Phi(\theta_i,\theta_i) = 1$ \Snote{but the above expression has $\widehat{\Phi}$}). As $a_i$ is initially $0$ when sampling $\theta_i$, we conclude that we can find $\theta_i$ such that
%
...
Furthermore, let $L$ as in \eqref{errLoss} with potential $\Phi_m$. Then, for all $ 1 > \epsilon > 0$, we can choose $m = O(poly(d)/\epsilon)$ such that if $(\boldsymbol{a,\theta}) \in \mathcal{M}_{\epsilon^2/(4d)}$ and $\|a\| \leq poly(d)$, then each $(a_i, \theta_i)$ must satisfy at least one of the following conditions:
