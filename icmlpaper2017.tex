...
$L(\boldsymbol{a,\theta}) = L(\boldsymbol{0,\theta})$ or $\theta_i = w_j$ for some $i,j$. 
\end{restatable}
\begin{proof}
...
Therefore, if $\theta_i \neq w_j$ for all $i,j$, then we may partition
...
\begin{observation}
We can initialize $\boldsymbol{a,\theta}$ such that $L(\boldsymbol{a,\theta}) < L(0,\boldsymbol{\theta})$ and continuous gradient descent will enforce the inequality as the $L$ is non-increasing.
\end{observation}
...
\begin{lemma}
For any $m$, we can construct a realizable $m$-almost 1-harmonic potential, $\Phi_m$, that corresponds to a polynomial $\sigma$.
\end{lemma}

\Snote{We should generalize this proof to almost $\lambda$-harmonic?}
\begin{proof}
...
Therefore, we get the following recurrence: $c_n (n(n-1) + (d-1)*n + 1) = c_{n+2} (n+2)(n+1)$. Let $c_0 = 1, c_1 = 0$, then let the recurrence hold for $n=0,...,m-2$ for some $m$ even. For $i > m$, we let $c_i = 0$. 
\begin{align*}
\Delta_\alpha h & = h''(\cos\alpha)(\sin \alpha)^{2} - (d-1)\cos(\alpha)h'(\cos\alpha) \\
&= h(\cos(\alpha)) + k_m(\cos\alpha)^m
\end{align*}
Where $k_m = m^2 + (d-2)m + 1$. Notice that $f(x)$ has \Snote{This is
  not $f$ anymore} non-negative Taylor coefficients and is a bounded
polynomial, so $f$ is a realizable potential function by Theorem
\ref{thm:rotReal}, with corresponding
$\sigma(x) = \sum_{i} \sqrt{c_i} H_i(x)$, where $H_i(x)$ is the i-th
normalized Hermite polynomial. We can also choose $m$ to be odd by
setting $c_0 = 0, c_1 = 1$. Now, by normalizing
$\Phi_m(\theta, w) = h(\theta^Tw)/h(1)$, have
\[|\Delta_\theta \Phi_m(\theta ,w) - \Phi_m(\theta,w) | \leq \frac{k_m}{h(1)} |\theta^Tw|^m \leq |\theta^Tw|^m\]
Thus, can construct a realizable $m$-almost $1$-Harmonic potential for any $m$.
 \end{proof}

\begin{lemma}\label{bounded}
Let $L$ be as in Eq.~\eqref{errLoss} and let $b_1,...,b_k$ be reals bounded by $poly(d)$ and $|\Phi|\leq poly(d)$ on $\mathcal{M}$. Let $\mathcal{A}$ be a gradient-type algorithm such that $\mathcal{A}(L)$ enforces that $L$ is non-increasing with high probability. Then, for $\lambda \geq 0$, $\mathcal{A}(L+ \lambda \|{\bf a}\|^2)$ initialized at ${\bf a} = 0$ will enforce $\|a\| \leq poly(d)/\lambda$.
\end{lemma}

\begin{proof}
Notice that $L({\bf a}, 0) \leq poly(d)$ \Snote{Not sure you mean
  (a,0), and why is this true? There has to be a dependence on $k$} and by the properties of
$\mathcal{A}$, with high probability, $L + \lambda \|{\bf a}\|^2 \leq
poly(d)$. Since $L \geq 0$, our claim follows. \Snote{We're applying
  the claim to $L + \lambda \norm {a}^2$ but the assumption is about
  $L.$ Just assume that the algorithm never increases (any) loss
  function with high probability.}
\end{proof}

\Snote{Why is $\Phi(u, v)$ bounded by $\poly(d)$ for all $u,v$?. Also,
use something other than $\lambda,$ maybe $\beta$}
\begin{restatable}{lemma}{eigConv}
\label{eigConv}
Let $\mathcal{M} = S^{d-1}$ and let $b_1,...,b_k$ be reals bounded by $poly(d)$. Furthermore, let $L$ be as in \eqref{errLoss} with potential $\Phi_m$ and let $G = L + \|a\|^2$. Then, for all $ 1 > \epsilon > 0$, there exists $m = O(poly(d)/\epsilon)$ such that if $(\boldsymbol{a,\theta}) \in \mathcal{M}_{poly(\epsilon,1/d)}$, then for all $i$, either there exists $j$ such that $|w_j^T\theta_i| > 1- \epsilon$ or $|a_i| < \epsilon/k$. 
\end{restatable}
\Snote{In this theorem, and everywhere below, can you replace $\Phi$
  by $\Phi_m$ whenever we're talking of the specific potential.}
%
\begin{proof}
  The proof is similar to Theorem \ref{EigStrict}. We first show a
  weaker result: each $(a_i,\theta_i)$ must satisfy (i) there exists
  $j$ such that $|w_j^T\theta_i| > 1-\epsilon$, or (ii)
  $|a_i| < \epsilon$, or (iii) there exists $j\neq i$ such that
  $|\theta_j^T\theta_i| > 1-\epsilon$. Assume that
  $(\boldsymbol{a,\theta}) \in \mathcal{M}_{\epsilon^2/d}$ and we have
  a $(a_i,\theta_i)$ that does not satisfy (i), (ii), (iii). Then,
%
  \[ \abs{\pd{G}{a_i}} = \abs{2\sum_{j=1}^k b_j \Phi(\theta_i^T w_j) +
    2\sum_{j: j\neq i} a_j \Phi(\theta_i^T\theta_j) + (2\Phi(1) +
    2)a_i} \leq \epsilon.\]
...
+ k_m(\theta_i^Tw_j)^m)  \\
& \qquad +  2\sum_{j: j\neq i} a_i
  a_j(-\Phi(\theta_i^T\theta_j) + k_m(\theta_i^T\theta_j)^m)
\end{align*}
\Snote{Why do we have a $k_m$ here?}

Therefore,
%
\begin{align*}
& |\Delta_{\theta_i}L + (2\Phi(1) + 2)a_i^2| \leq \epsilon |a_i|  \\
& \qquad +  2  k_m|a_i| \abs{\sum_{j=1}^k b_j (\theta_i^Tw_j)^m +
  \sum_{j: j\neq i}  a_j(\theta_i^T\theta_j)^m}
\end{align*}

Since (i), (iii) does not hold and $\|a\| \leq poly(d)$ by Lemma
\ref{bounded}, \Snote{Needs to be updated once we have a updated
  lemma, depends on $k, m,$ and needs to reference the lemma about $\Phi$} we can choose $m = O(\frac{1}{\epsilon}\log(d))$ large enough such that 
%
\begin{align*}
 2k_m |\sum_{j=1}^k b_j (\theta_i^Tw_j)^m +  \sum_{j\neq i}
  a_j(\theta_i^T\theta_j)^m| & \leq  2poly(d,k,m)e^{-\epsilon m} \\
& \leq \epsilon
\end{align*}
...
%
\begin{align*}
\Delta_{\theta_i} L \leq -(2\Phi(1) + 2)a_i^2 +\epsilon |a_i|+
  \epsilon|a_i| & \leq |a_i|(-4\epsilon + 2\epsilon)  \\
& \leq -\epsilon^2.
\end{align*}

Since $\Delta_{\theta_i}L$ is a sum of $d$ eigenvalues, this implies
that $(\boldsymbol{a,\theta})$ is $\epsilon^2/d$-strict, contradicting
that it is in $\mathcal{M}_{\epsilon^2/d}$.  

Now, consider all $\theta_i$ such that only condition (iii) holds
\Snote{Do you mean to say all $\theta_i$ such that iii holds, but i
  and ii don't? If yes, make that clear}. WLOG, assume that these are
$\theta_1,...,\theta_l$, for some $l \leq k$. Furthermore, assume that
$|\theta_i^T\theta_j| \leq 1-\epsilon$ for $i \leq l, j\geq l+1$.
...
& + k_m(\theta_i^Tw_j)^m) + 2\sum_{i=1}^l\sum_{j= l+1}^k a_i
a_j(-\Phi(\theta_i^T\theta_j) + k_m(\theta_i^T\theta_j)^m)
\end{align*}
...
& \qquad +  2|a_i| k_m\sum_{i=1}^l|\sum_{j=1}^k b_j (\theta_i^Tw_j)^m
...
%
\begin{align*}
 2|a_i| k_m\sum_{i=1}^l|\sum_{j=1}^k b_j (\theta_i^Tw_j)^m +
  \sum_{j=l+1}^k  a_j(\theta_i^T\theta_j)^m| \\
...
& =\sum_{i=1}^l a_i^2\Phi(\theta_i^T\theta_i) + \sum_{i \neq j}^l
  a_ia_j\Phi(\theta_i^T\theta_j)
\end{align*}
%
Since $D \geq 0$ and combined with the fact that (ii) does not hold,
%
\begin{align*}
\Delta_{\theta_i} L & \leq -2a_i^2 +\epsilon |a_i|+(\epsilon/2)|a_i| \\
& \leq  |a_i|(-2\epsilon + 3\epsilon/2) \leq -\epsilon^2/4
\end{align*}

This implies that $(\boldsymbol{a,\theta})$ is $\epsilon^2/(4d)$-strict, contradicting that it is in $\mathcal{M}_{\epsilon^2/(4d)}$. 

Finally, there exists $\theta_{j_1},...\theta_{j_l}$ such that
$|\theta_i^T\theta_{j_1}| > 1-\epsilon$ and
$|\theta_{j_{i}}^T\theta_{j_{i+1}}| > 1-\epsilon$, and
$|\theta_{j_l}^Tw_j| > 1-\epsilon$ for some $w_j$. Therefore,
$|\theta_i^Tw_j| > 1- (l+1)\sqrt{\epsilon}$. By choosing
$\epsilon' = \epsilon/k$, we get our claim.
\end{proof}

\begin{theorem}\label{eigRes}
Assume the conditions of \ref{eigConv}. If $\sqrt{L({\bf a, \boldsymbol{\theta}})} < \sqrt{L(0,\boldsymbol{\theta})} - \epsilon$ and $(\boldsymbol{a,\theta}) \in \mathcal{M}_{poly(\epsilon,1/d)}$, then there exists some $i, j$ such that $\theta_i$ is in an $\epsilon$-neighborhood of $w_j$.
\end{theorem}
 
 \begin{proof}
By Lemma \ref{eigConv}, if there does not exists $i, j$ such that
$|w_j^T\theta_i| >  1-\epsilon$ is if for all $\theta_i$, then $|a_i|
< \epsilon/k$ for all $i$. Now, for a integrable function $f(x)$, $\|
f\|_X = \sqrt{E_X[f(X)^2]}$ is a norm. Therefore, if $f(x) = \sum_i
b_i \sigma(w_i,x)$ be our true target function, we conclude that by
triangle inequality
\begin{align*}
\sqrt{L(\boldsymbol{a,\theta})} & = \|\sum_{i=1}^k a_i \sigma(\theta_i,x) - f(x)\|_X \\
& \geq \|f(x)\|_X\ - \sum_{i=1}^k \|a_i\sigma(\theta_i,x) \|_X \geq
  \sqrt{L(0,\boldsymbol{\theta})} - \epsilon
\end{align*}
...
\begin{align*}
 %
  \expt[ L(a_1,\theta_1)] - L(0,0) & = \expt\left [\min_{a_1} a_1^2 +
