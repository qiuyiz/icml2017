...
\[\mathcal{M}_\epsilon = \left\{x\in \mathcal{M} \Big| \|\nabla L(x)\|
  ...
  -\epsilon\right\}\]
\Snote{This is not what the theorem states. There are lots of
  assumptions on $L$ that don't factor into the claim here. Try to
  write a formal version of the theorem.}
...
- w\|^2/2)$. In $\R^d$, the Laplacian is $\Delta \Phi = ( c\|\theta -
w\|^2 -d ) \exp(-c\|\theta - w\|^2/2)$, which becomes positive when
$\|\theta - w \|^2 \geq d/c$. Thus, $\Phi$ is strictly subharmonic
outside a ball of radius $\sqrt{d/c}$.  \Snote{Is it realizeable? What
  is the $\sigma$? Please focus on the $\sigma$}
...
function, so we can show convergence similarly. \Snote{This paragraph
  also seems completely out of place.}
%
\begin{theorem}\label{gaussStrict}
\label{GaussStrict}
Let $\mathcal{M} = \R^d$ and $\Phi(\theta,w) = e^{-c\|a-b\|^2/2}$
\Snote{what are a,b ? } and Assumption \ref{outputFixed} holds. Let $L$ be as in \eqref{errFixed}, then if $c = d/\epsilon$ and $\boldsymbol{a,\theta} \in \mathcal{M}_{e^{-poly(d,1/\epsilon)}}$, then there exists $i, j$ such that $\| \theta_i - w_j \|^2 \leq \epsilon$.
\end{theorem}
\Snote{Does this theorem need to be here?}
...
all $\theta, w\in \Omega$, \Snote{What is $\Omega$?} $|\Delta_{\theta}\Phi(\theta, w) - \lambda \Phi (\theta,w)| \leq |\theta^Tw|^m$ 
\end{definition}
...
\end{lemma} \Snote{The focus should be $\sigma$ everywhere in the
  section. We should say here e.g. there exists a bounded sigma on the
  sphere such that the corresponding potential is $m$-almost
  1-harmonic ... }
...
\end{align*}
\Snote{$f$ and $\theta$ are overloaded here}
...
Therefore, we get the following recurrence: $c_n (n(n-1) + (d-1)*n +
1) = c_{n+2} (n+2)(n+1)$. Let $c_0 = 1, c_1 = 0$, then we run this
recurrence holds $n=0,...,N-2$ for some $N$ even and set $c_{N+i} = 0$
for $i > 0$, then \Snote{Reword this last line carefully.}
\begin{align*}
\Delta f & = f''(\cos\theta)(\sin \theta)^{2} - (d-1)\cos(\theta)f'(\cos\theta) \\
&= f(\cos(x)) + k_N(\cos\theta)^N
\end{align*}

Where $k_N = N^2 + (d-2)N + 1$. Notice that $f(x)$ has non-negative
Taylor coefficients and is a bounded polynomial, so $f$ is a
realizable potential function by Theorem \ref{thm:rotReal}. We can
also choose $N$ to be odd by setting $c_0 = 0, c_1 = 1$. Therefore, we
can construct a realizable $m$-almost $1$-Harmonic potential for any
$m$. \Snote{This does not satisfy the $m$-almost definition because
  $k_N > 1.$} \Snote{Also, we have assumed in the paper that the
  potentials are normalized with $\Phi( \theta, \theta) = 1$ thus, we
  should have $f(1) = 1.$ We need to fix the normalization.}
\end{proof}
...
Notice that $L({\bf a}, 0) \leq poly(d)$ \Snote{Not sure you mean
  (a,0), and why is this true?} and by the properties of
...
\Snote{Why is $\Phi(u, v)$ bounded by $\poly(d)$ for all $u,v$?.}
