...
%
\begin{proof}
The proof is similar to Theorem \ref{EigStrict}. Consider all $\theta_i$ such that $|a_i| > \delta$ and $|w_j^T\theta_i| \leq 1-\epsilon$. WLOG, assume that these are
...
Let $(\boldsymbol{a,\theta}) \in \mathcal{M}_{\delta^2/(4d)}$. Then, the optimality conditions on ${\bf a}$ are as usual:
%
  \[ \abs{\pd{H}{a_i}} = \abs{2\sum_{j=1}^k b_j \Phi_m(\theta_i^T w_j) +
    2\sum_{j: j\neq i} a_j \Phi_m(\theta_i^T\theta_j) + (2\Phi_m(1) +
    2)a_i} \leq \delta \]
%
And we have,
%
\begin{align*}
\Delta_{{\bf v}} H & =  2\sum_{i=1}^l\sum_{j=1}^k a_i b_j  (-\Phi_m(\theta_i^T w_j)  + (\theta_i^Tw_j)^m) \\
+ &2\sum_{i=1}^l\sum_{j= l+1}^k a_ia_j(-\Phi_m(\theta_i^T\theta_j) + (\theta_i^T\theta_j)^m)
\end{align*}
%
Together, we get
%
...
Again, since these inner products are assumed to be less than $1-\epsilon$ and  and $\|a\| \leq poly(d)$ by Lemma \ref{bounded}, we can choose $m = O(\frac{1}{\epsilon}\log(d)\log(1/\delta))$ large enough such that 
%
\begin{align*}
...
Assume the conditions of \ref{eigConv}. If $\sqrt{G({\bf a, \boldsymbol{\theta}})} < \sqrt{G(0,\boldsymbol{\theta})} - \delta$ and $(\boldsymbol{a,\theta}) \in \mathcal{M}_{\delta^2/(4poly(d))}$, then there exists some $i, j$ such that $\theta_i$ is in an $\epsilon$-neighborhood of $w_j$.
\end{theorem}
 
 \begin{proof}
By Lemma \ref{eigConv}, if there does not exists $i, j$ such that
$|w_j^T\theta_i| >  1-\epsilon$ is if for all $\theta_i$, then $|a_i|
< \delta/poly(d)$ for all $i$. Now, for a integrable function $f(x)$, $\|
...
Let $h(\theta) = \sum_i b_i \Phi(\theta, w_j)$. Since $a_i,b_i, \Phi$ is $poly(d)$-bounded, our random variable $g(\theta)$ is $poly(d)$-bounded. From Hoeffding bounds, we see that after $poly(d,1/\delta)$ samples, we know that $\hat{\expt}h(\theta)^2 \geq 2\delta\sqrt{G(0,0)}$, where $\hat{\expt}$ denotes the empirical average over $\theta$. Therefore, we must have found some $\theta$ such that $h(\theta)^2 \geq 2\delta \sqrt{G(0,0)}$. Therefore, $G(\boldsymbol{a,\theta}) - G(0,0) \leq 2\delta \sqrt{G(0,0)}$ and so $\sqrt{G(\boldsymbol{a,\theta})} \leq \sqrt{G(\boldsymbol{0,\theta})} - \delta$.
 \end{proof}
