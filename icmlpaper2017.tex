...
\begin{theorem}
Let $\mathcal{M} = S^{d-1}$ and let $b_1,...,b_k$ be  such that $||b||_2$ is $\Omega(1)$ and $O(poly(d))$, and
$w_1,..,w_k$ are randomly chosen from $\mathcal{M}$. Then 
there exists an $m$ (that depends on $d$) so that for the potential $\Phi_m$ the following holds: we can chose an initial point $(a,\theta)$ so that with high probability after running Gradient Descent (Algorithm 1) with the regularized objective $G(\boldsymbol{a,\theta})$ if 
...
\Anote{ in terms of the corresponding activation function instead of the potential. }
\begin{theorem}
\label{PolyStrict}
