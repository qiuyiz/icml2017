...
\end{align*}
Where $k_m = m^2 + (d-2)m + 1$. Notice that $f(x)$ has non-negative Taylor coefficients and is a bounded polynomial, so $f$ is a realizable potential function by Theorem \ref{thm:rotReal}, with corresponding $\sigma(x) = \sum_{i} \sqrt{c_i} H_i(x)$, where $H_i(x)$ is the i-th normalized Hermite polynomial. We can also choose $m$ to be odd by setting $c_0 = 0, c_1 = 1$. Now, by normalizing $\Phi_m(\theta, w) = h(\theta^Tw)/h(1)$, have 
...
\[\geq \|f(x)\|_X\ - \sum_{i=1}^k \|a_i\sigma(\theta_i,x) \|_X \geq \sqrt{L(0,\boldsymbol{\theta})} - \epsilon\]
