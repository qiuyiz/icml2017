...
\end{align*}
Where $k_m = m^2 + (d-2)m + 1$. Notice that $f(x)$ has non-negative Taylor coefficients and is a bounded polynomial, so $f$ is a realizable potential function by Theorem \ref{thm:rotReal}, with corresponding $\sigma(x) = \sum_{i} \sqrt{c_i} H_i(x)$, where $H_i(x)$ is the i-th normalized Hermite polynomial. Now, by normalizing $\Phi_m(\theta, w) = h(\theta^Tw)/h(1)$, have 
...
%
\[\Delta_{\theta_i} G \leq -(2\Phi(1) + 2)a_i^2 +\epsilon |a_i|+ \epsilon|a_i| \leq |a_i|(-4\epsilon + 2\epsilon) \]
\[\leq -\epsilon^2\]

Since $\Delta_{\theta_i}G$ is a sum of $d$ eigenvalues, this implies that $(\boldsymbol{a,\theta})$ is $\epsilon^2/d$-strict, contradicting that it is in $\mathcal{M}_{\epsilon^2/d}$. 
...
\end{theorem}
 ...
 \begin{proof}
By Lemma \ref{eigConv}, if there does not exists $i, j$ such that $|w_j^T\theta_i| >  1-\epsilon$ is if for all $\theta_i$, then $|a_i| < \epsilon/k$ for all $i$. Now, for a integrable function $f(x)$, $\| f\|_X = \sqrt{E_X[f(X)^2]}$ is a norm. Therefore, if $f(x) = \sum_i b_i \sigma(w_i,x)$ be our true target function, we conclude that by triangle inequality

\[\sqrt{G(\boldsymbol{a,\theta})} \geq \|\sum_{i=1}^k a_i \sigma(\theta_i,x) - f(x)\|_X  \]

\[\geq \|f(x)\|_X\ - \sum_{i=1}^k \|a_i\sigma(\theta_i,x) \|_X \geq \sqrt{G(0,\boldsymbol{\theta})} - \epsilon\]
...
Then, we can initialize $\boldsymbol{a,\theta}$ such that $\sqrt{G({\bf a, \boldsymbol{\theta}})} < \sqrt{G(0,\boldsymbol{\theta})} - \epsilon$ with high probability.
 ...
 %
 \[ \expt[ G(a_1,\theta_1)] - G(0,0) = \expt\left [\min_{a_1} 2a_1^2 + 2\sum_{j=1}^k a_1 b_j\Phi(\theta_1,w_j)\right]\] 
 %
 \[ = \expt\left[\frac{1}{2}\left(  \sum_{j=1}^k b_j \Phi(\theta,w_j)\right)^2\right] \]
