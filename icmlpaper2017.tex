...
\end{align*}
%
We want $b$ to be approximately $1$-harmonic, so we want to approximately solve:
%
...
Therefore, we get the following recurrence: $c_n (n(n-1) + (d-1)*n + 1) = c_{n+2} (n+2)(n+1)$. Let $c_0 = 1, c_1 = 0$, then we run this recurrence holds $n=0,...,m-2$ for some $m$ even and set $c_{m+i} = 0$ for $i > 0$, then
\begin{align*}
\Delta f & = h''(\cos\alpha)(\sin \alpha)^{2} - (d-1)\cos(\alpha)h'(\cos\alpha) \\
&= h(\cos(x)) + k_m(\cos\alpha)^m
\end{align*}

Where $k_m = m^2 + (d-2)m + 1$. Notice that $f(x)$ has non-negative Taylor coefficients and is a bounded polynomial, so $f$ is a realizable potential function by Theorem \ref{thm:rotReal}, with corresponding $\sigma(x) = \sum_{i} \sqrt{c_i} H_i(x)$, where $H_i(x)$ is the i-th normalized Hermite polynomial. We can also choose $m$ to be odd by setting $c_0 = 0, c_1 = 1$. Now, by normalizing $\Phi_m(\theta, w) = h(\theta^Tw)/h(1)$, we can construct a realizable $m$-almost $1$-Harmonic potential for any $m$.
\end{proof}
