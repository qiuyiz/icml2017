...
\end{align*}
%
As $a_i$ is initially $0$ when sampling $\theta_i$, we conclude
...
Let $\Phi_m$ be the $(1,m)$-Harmonic potential in Theorem \ref{eigConv} with $m = O(\log(d)/\epsilon)$ and $m$ odd. We first consider the algorithm on node $\theta_1$ and claim that it will merge with some $w_j$ and then we will proceed with induction. 
