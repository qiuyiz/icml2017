...
\begin{theorem}
  ...
  randomly chosen from $\mathcal{M}$. Then for all
  $\epsilon \in (0,1),$ there exists $m, \delta$ (that depends on $d$)
  such that for the potential $\Phi_m$ the following holds: we can chose
  an initial point $(\boldsymbol{a^{(0)}, \theta^{(0)}})$ so that if
  after running gradient descent on the regularized objective
  $G(\boldsymbol{a,\theta}),$ we have
% with high
%   probability after running Gradient Descent (Algorithm 1) 
  $(\boldsymbol{a,\theta}) \in \mathcal{M}_{\delta^2/(4d)},$ then there
  exists an $i, j$ such that $|w_j^T\theta_i| > 1- k^2\epsilon,$
\end{theorem}
