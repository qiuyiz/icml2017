...
The gradient descent process for learning our neural network $f$ is equivalent to the motion of k electrons in the presence of k fixed protons where the force between any pair of charges is given by a potential function that depends on $\sigma$.
\end{theorem}
%
The charges reside in $R^d$. The protons are fixed at locations $w_1,..,w_k$. The electrons are at positions $\theta_1,..,\theta_k$ and can move: the total force on each charge is the sum of the pairwise forces, determined by the gradient of the potential function $\Phi(\theta, w) = E_{X\sim \mathcal{D}}[ \sigma(X,\theta) \sigma(X,w)]$.  
...
For some carefully chosen potential function with regularization, at convergence of running the SGD algorithm, we learn at least one of the hidden weights of the target neural network in at most $\poly(d,1/\epsilon)$ iterations.
\end{theorem}
%
...
$\widehat{f} \in \mathcal{C}_{\sigma, k}$ such that with high probability, $\theta_i$ is in an $\epsilon$-neighborhood of $w_j$ for some (or all) $i, j$ in time $\poly(d,1/\epsilon)$ or some
...
\end{definition}
%
\begin{restatable}{theorem}\label{EPDyn}
  Let $\Phi$ be a symmetric potential. Running continuous gradient
  descent on \eqref{errLoss} with respect to $\theta$, initialized at
...
Let $L :\Omega \to \R$ be a twice differentiable function such that $|L(x)| \leq B, \|\nabla^2 L(x)\| \leq C,\|\nabla^2L(x) -\nabla^2L(y)\| \leq \rho\|x - y\|$ for all $x,y\in\Omega$. Also, assume that we access to stochastic function $\widehat{L}$ such that \Anote{are you asking for all three quantities to be less than $\epsilon/3$? the first two quantities have no conditions imposed on them.} $\|\widehat{L}(x) -L(x)\|, \|\nabla \widehat{L}(x) = L(x)\|, \|\nabla^2\widehat{L}(x) -\nabla^2 L(x)\| \leq \epsilon/3$ for all $x\in \Omega$. \Anote{are you asking for this almost surely? if so, it's worth specifying.}

\Anote{you reference Algorithm \ref{SGD}, but this algorithm is only defined in the appendix only. bring it into this section.} 
...
\end{lemma}
\Snote{Why do we only look at rotationally invariant potentials? Do we
prove it's invariant, or do we assume so (I don't see it written) ?}
\begin{proof}
...
$1-\epsilon$ and and $\|a\| \leq \poly(d)$ by $G(\boldsymbol{a,\theta}) \leq G(0,\boldsymbol{\theta})$ and Lemma \ref{bounded}, we
...
\end{proof}
 ...
 \begin{lemma} For  the almost $\lambda$ harmonic function, for random $\theta$ on $S^{d-1}$, $E[\Phi(w,\theta)^2] \geq \Omega((1/m)^d)$.

If $w_1,..,w_k$ are random $\expt\left[\left(  \sum_{j=1}^k b_j \Phi(\theta,w_j)\right)^2\right] \geq \|b\|_2^2 \Omega((1/m)^d)$
\end{lemma}
...
For the second part, note that if $w_1,..,w_k$  are random  when we expand the square in the expectation the cross terms will disappear as the $w_i$'s are independent and because  $E[\Phi(w_i,\theta)]=0$,
 \[ \expt\left[\left(  \sum_{j=1}^k b_j \Phi(\theta,w_j)\right)^2\right] \ge k\Omega((1/m)^d)\]
...
%
\Anote{this theorem requires assumption 1, which is not of practical relevance to the paper. you should remove this theorem.}
...
\end{equation}
%\Anote{this theorem is about $\theta \in R$. it is not of practical relevance. you should remove it.}
\begin{theorem}
\label{SignConv}
...
\Anote{this theorem applies to a two layer network with two hidden layers. unless you're going to build a significant result on top of this in this paper, it is not of any independent practical interest. you should remove it.}
...
\Anote{this and the next theorem seem relevant enough that they're worth preserving. since (i believe) this potential is realizable, you should state this theorem and the next in terms of the corresponding activation function instead of the potential. }
\begin{theorem}
\label{PolyStrict}
