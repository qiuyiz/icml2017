...
\nabla_{\theta_i}\Phi(\theta_i,\theta_j) - \sum_{j=1}^k
a_ib_j\nabla_{\theta_i} \Phi(\theta_i,w_j)\] \Snote{Isn't there a 2 in
  the second term?}
...
\begin{definition}
  ...
  $\norm{v} \le \epsilon.$
\end{definition} \Snote{Define a critical point.}

\begin{fact}
  % Let $\Omega \subseteq \R^d$ and consider a function
  % $f:\Omega \to \R$.  
Let $x^*$ be a critical point of a function $f :
  \Omega \to \R$ such that
  $f$ is differentiable at $x^*.$ Then, $x^*$ is a local minima iff
  $\lambda_{min}(\nabla^2 f(x^*)) \geq 0$ and it is a strict local
  minimum iff $\lambda_{min}(\nabla^2 f(x^*)) > 0$.
\end{fact}
%
\begin{theorem}[Earnshaw's Theorem. See~\cite{arnold1985mathematical}]
\label{Earnshaw} 
Let $\mathcal{M} = \R^d$ and let $\Phi$ be harmonic and $\overline{L}$
be as in $\eqref{errSimp}$. Then, $\overline{L}$ admits no
...
\end{align*}
which is a contradiction. \Snote{I think there's no 2 in the first
  term since you include all $i \neq j$}
\end{proof}
...
\cite{arnold1985mathematical}). Therefore, at convergence, we should
expect $\theta_i = w_{\pi(i)}$ for some permutation $\pi$, as long as
$\theta_i$ are all distinct. \Snote{Need to fix the zero hessian case,
  and the claim for the permutation.}
...
\end{definition} \Snote{Has to include a differentiability assumption.}

An example of such a potential is $\Phi(\theta, w) = \|\theta -w \|^{2-d-\epsilon}$ for any $\epsilon > 0$. Note that although this potential is unbounded for most $d$, it is bounded when $d = 1$. Furthermore, the sign of the output weights $a_i, b_i$ matter in determining the sign of the Laplacian of our loss function. Therefore, we need to make suitable assumptions in this framework.
%
\begin{assumption}
\label{outputFixed}
...
$\theta_i = \theta_j$ for some $i,j$, then separating them shows that
we are not at a local minima since $\Phi(\theta_i,\theta_j) = \infty$
and finite elsewhere. \Snote{This claim needs to be fixed, there might
  be other pairs that coincide. We have to separate all the pairs.}
...
The above
technique generalizes to $\Phi$ being rotationally invariant case by
working in spherical coordinates and correlated translations are
simply rotations, with full details in the supplementary
material. \Snote{Where is the proof of the rotationally invariant case?}
\end{proof}
%
\begin{corollary}
...
\end{corollary} \Snote{This is not obvious. You have to handle the
  case that $\theta_i = \theta_j$ for some $i,j.$ A bigger question :
  Is there ANY such potential? forget even a realizeable one.}
...
We note that there are realizable versions of these potentials; for example $\Phi(a,b) = e^{-\|a-b\|_1}$ in $\R^1$. In Section 4, we construct a realizable approximation of these $\lambda$-harmonic potentials on $S^{d-1}$ that retains good convergence behavior.
%
\begin{restatable}{theorem}{eigStrict}
\label{EigStrict}
Let $\Phi$ be $\lambda$-harmonic and $L$ be as in \ref{errLoss}. Then, $L$ admits no local minima $\boldsymbol{(a,\theta)}$, except when $\hat{f} = 0$ or $\theta_i = w_j$ for some $i,j$. 
\end{restatable}
\begin{proof}
...
&= -2\lambda a_i^2
\end{align*}
\Snote{I think the definition of $\Delta L$ is incorrect. There is no
  2 in the first term.}
If $a_i^2 \neq 0$, then we conclude that the Laplacian is strictly
negative, so we are not at a local minimum. \Snote{ How are we
  handling the case $a_i = 0$?}

Otherwise, if all $\theta_i$ are not distinct, let
$\theta_1 = \theta_2 =... = \theta_l$ be identical and
$\theta_1 \neq w_j$ for any $j$. \Snote{Aren't we assuming that
  $\theta_j \neq \theta_1$ for $j > l$?} We consider changing all
...
\begin{align*}
\Delta_v H & = \lambda\sum_{i=1}^l a_i \left(\sum_{j=1}^k b_j
...
%
& = -2 \lambda\expt\left[ \left(\sum_{i=1}^l a_i \sigma(\theta_i,
  X)\right)^2\right] = -2 \lambda\left(\sum_{i=1}^l a_i\right)^2
\end{align*}

If $\sum_{i=1}^l a_i \neq 0$, then we conclude that the Laplacian is strictly negative, so we are not at a local minimum. Therefore, if does not exist $\theta_i = w_j$ for some $i,j$, then we partition $[k]$ into $S_1,...,S_r$ such that each $S_i$, $\theta_{u} = \theta_v$ for $u, v \in S_i$ and $\sum_{u \in S_i} a_u = 0$. In this case, $\hat{f} = 0$.
\end{proof} 
...
gradient descent (SGD) in standard backpropagation. To derive finite
runtime bounds, we need to address three technical details: 1)
realizable potentials are only approximately strict subharmonic or
$\lambda$-harmonic, 2) the variance of the stochastic gradient and 3)
SGD should escape local minima in a bounded (hopefully polynomial)
amount of time. The second and third detail are analyzed with standard
techniques in optimization and statistics: the former requires
generalization bounds for neural networks and the latter requires a
lower bound on the negative curvature, which leads to the follow
definition.
%
\begin{definition} 
...
Using the Laplacian formula on $S^{d-1}$ gives \Snote{Give a reference
  for this formula}:
\begin{align*}
...
This is a contradiction, so we conclude that there must exist $i, j$ such that $\theta_i$ is in a neighborhood of $\theta_j$.
 ...
 \begin{observation}
\[ \expt\left[\left(  \sum_{j=1}^k b_j \Phi(\theta_i,w_j)\right)^2\right] < \epsilon\] 
 \end{observation}
