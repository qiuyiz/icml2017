...
%
\begin{proof}
  Note that $\Phi_m$ is a degree $m$ polynomial with non-negative
  coefficients and it only has odd-degree terms. Since it is odd,
  $\expt[\Phi_m(\theta^Tw_i)] = 0$ and the cross terms in the
  expansion of the square vanish:
%
  \[ \expt\left[\left( \sum_{j=1}^k b_j
      \Phi_m(\theta^Tw_j)\right)^2\right] = \sum_{j=1}^k b_j^2
  \expt[\Phi_m(\theta^Tw_j)^2]\]
...
\begin{align*}
  ...
  & \qquad = \sum_{i,j \,odd} c_ic_j \expt[(\theta^Tw_j)^{i+j}] \geq \sum_{i,j
    \,odd}c_i c_j \Omega(1/d)^{i+j}  \\
  & \qquad \ge\Omega(1/d)^{2m},
\end{align*}

where we used the fact that
$\sum_{i,j} c_ic_j = (\sum_i c_i)^2 = \Phi_m(1)^2 = 1$ and
$E[(\theta^Tw_j)^{n}] \geq \Omega(1/d^{n})$, which holds by noticing
that with high probability $\theta^T w_j =
\Omega(1/\sqrt{d})$.
\Snote{Again, the last claim is not correct. It's only true with
  constant probability. We need to find a good way to prove this lemma
  formally. }
\end{proof}
...
We have
%
\[L(\boldsymbol{0,0}) =  \sum_{i=1}^k\sum_{j=1}^k b_i b_j \Phi(w_i, w_j) \]

Therefore, we can find $i$, such that
$|\sum_{j=1}^k b_j \Phi(w_i, w_j)| \geq
L(\boldsymbol{0,0})/(kb_i)$.
Since $b_i$ is $\poly(d)$-bounded and $\Phi$ is
$m$-Lipschitz~\Snote{where do we prove $m$-Lipschitz?}, we see
...
neighborhood is $e^{-O(d)}$, so we conclude. \Snote{How does the claim
...
\begin{theorem}
  ...
  $G(\boldsymbol{a,\theta})$, there exists an $i, j$ such that $|w_j^T\theta_i| > 1- k^2\epsilon$ in $e^{O(d)}poly(1/\epsilon)$ iteration complexity and $d^{\poly(d, 1/\epsilon)}$ sample complexity.
\end{theorem}

\begin{proof}
We choose $m = O(\log d \log (1/\delta)/\epsilon)$. Combining Observation \ref{initialize} and Lemma \ref{bigVariance}, we can initialize $\boldsymbol{a,\theta}$ such that $G(\boldsymbol{a,\theta}) - G(0,0) \leq 2\delta \sqrt{G(0,0)}$ for $\delta = e^{-O(d)}$.

Next, to use Theorem \ref{strongConverge}, we check the regularity conditions. By assumption, we can choose $B, L, \rho$ to be $\poly(d,1/\epsilon)$ since $\Phi$ and the second and third partials of $\Phi$ are all bounded by $\poly(d,1/\epsilon)$ in $\mathcal{M}$. Furthermore, our activation function $\sigma$ and its derivatives are bounded in magnitude by $c|x|^{m}$, where $m = O(\log d \log (1/\delta)/\epsilon)$ and by \cite{Hermite}, we can let $c = O(m^m)$. By Theorem \ref{genErrBound}, with high probability, we can construct a stochastic oracle up to $\epsilon$ error with sample complexity $d^{poly(d,1/\epsilon)}$. Therefore, we conclude by Theorem \ref{strongConverge} that we are in $\mathcal{M}_{G,\delta^2/(4\poly(d))}$ in $\poly(e^{O(d)},1/\epsilon)$ iterations. Finally, we conclude by Theorem \ref{eigRes}.
\end{proof}
