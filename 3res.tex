...
Therefore, we can find $i$, such that $|\sum_{j=1}^k b_j \Phi(w_i, w_j)| \geq L(\boldsymbol{0,0})/(kb_i)$. Since $b_i$ is $\poly(d)$-bounded and $\Phi$ is $m$-Lipschitz, we see that in a $\Omega(1/poly(d,m))$-neighborhood of $w_i$,

\[|\sum_{j=1}^k b_j \Phi(\theta, w_j)| \geq L(\boldsymbol{0,0})/poly(d) \]

The probability $\theta$ randomly chosen would lie in this neighborhood is $e^{-O(d)}$, so we conclude.
\end{proof}

\begin{theorem}
  Let $\mathcal{M} = S^{d-1}$ and let $b_1,...,b_k$ be such that
  $||{\bf b}||_2$ is $\Omega(1)$ and $O(poly(d))$, and $w_1,..,w_k$ are
