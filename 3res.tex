...
\begin{theorem}\label{eigSGD}
  Let $\mathcal{M} = S^{d-1}$ and $b_1,...,b_k$ be bounded by $\poly(d)$ and $w_1,..,w_k$ are randomly chosen from $\mathcal{M}$.  \Snote{Aren't $b_i$ from the
    sphere, and hence have $\norm{b_i}_2 = 1$?}\Rnote{Actually, it turns out now that they just have to be bounded by poly(d)}
