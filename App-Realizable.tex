...
And for $r \geq \epsilon$, we see that $|\Phi_\epsilon(r)| = |\Phi(r)| \leq |p(r)|\frac{e^{-\sqrt{\lambda}r}}{r^{d-2}} = (1+r\sqrt{\lambda})^de^{-\sqrt{\lambda}r}r^{2-d}$.


Finally, we consider the normalized potential: $\widetilde{\Phi}_\epsilon = {\Phi}_\epsilon/{\Phi}_\epsilon(0)$. Note that since ${\Phi}_\epsilon$ is monotonically decreasing, we can lower bound ${\Phi}_\epsilon(0) \geq {\Phi}_\epsilon(\epsilon) \geq e^{-\sqrt{\lambda}}$. Therefore, we can derive the following upper bounds: $|\widetilde{\Phi}_\epsilon^{(k)}(r)| \leq 3(2d + \sqrt{\lambda})^{2d} \epsilon^{-2d}e^{\sqrt{\lambda}}$ and for $r \geq \epsilon$, $|\widetilde{\Phi}_\epsilon(r)| \leq (1+r\sqrt{\lambda})^de^{\sqrt{\lambda}(1-r)}(r)^{2-d}$. 

And lastly, we derive some lower bounds on $\widetilde{\Phi}_\epsilon$ for $r \geq \epsilon$, by noting that $\widetilde{\Phi}_\epsilon(r) \geq {\Phi}_\epsilon(r)(2d+\sqrt{\lambda})^{-2d}\epsilon^{2d}/3  \geq e^{-\sqrt{\lambda}r}r^{2-d}(2d+\sqrt{\lambda})^{-2d}\epsilon^{2d}/3$
\end{proof}
